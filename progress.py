...
def draw_progress_bar(stdscr, current, total, width):
    if total == 0:
        percentage = 0
    else:
    ...
    display_str = f"[{bar}] {current}/{total} ({percentage:.0f}%)"
    return display_str
...
def display_tasks(stdscr, tasks):
    ...
    start_line = 3
    max_tasks_display = (h - 5 - start_line) // 2
    ...
    for i, task in enumerate(tasks):
        ...
        task_info = f"{task['hash']} {task['type']:<9} {
            task['tag']:<9} {task['name'][:w-30]}"
    ...
    stdscr.addstr(h - 3, 0, "--------------------")
    stdscr.addstr(
        h - 2, 0, "Commands: [i]nsert, [a]dd progress, [d]elete, [q]uit")
...
def handle_insert_task(stdscr, app_state):
    ...
    for i in range(input_display_start_line, input_display_start_line + 5):
        stdscr.move(i, 0)
        stdscr.clrtoeol()
...
def handle_add_progress(stdscr, app_state):
    ...
    for i in range(input_display_start_line, input_display_start_line + 3):
        stdscr.move(i, 0)
        stdscr.clrtoeol()

    hash_input = get_curses_input(
        stdscr, input_display_start_line, 0, "Enter task hash (partial allowed): ").strip()
    change_str = get_curses_input(
        stdscr, input_display_start_line + 1, 0, "Enter change (+N, -N): ").strip()
...
def handle_delete_task(stdscr, app_state):
    ...
    for i in range(input_display_start_line, input_display_start_line + 2):
        stdscr.move(i, 0)
        stdscr.clrtoeol()

    hash_input = get_curses_input(stdscr, input_display_start_line,
                                  0, "Enter task hash to delete (partial allowed): ").strip()
